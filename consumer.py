...
import json
from music.models import UserPlaylistMapping
import retry

def user_update(msg):
  if msg:
    data = json.loads(msg.decode())
    user = UserPlaylistMapping.objects.filter(user_id = data.get('id')).last()
    if user:
        user.username = data.get('username')
        user.save()
    else:
         UserPlaylisotMapping.objects.create(user_id = data.get('id'),
